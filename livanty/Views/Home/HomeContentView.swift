...
struct HomeContentView: View {
    ...
    @State private var showTapAnimation = true
    @State private var tapAnimationOffset: CGFloat = 0

    // Navigation state
    @State private var navigationPath = NavigationPath()

    @State private var timerInterval: Double = 0.05
    let achievementTimer = Timer.publish(every: 4, on: .main, in: .common)
        .autoconnect()
    ...
    let personalCards = [
        PersonalCard(
            ...
            backgroundPatternIcons: [
                "figure.badminton", "figure.run", "figure.basketball",
            ],
    ...
    private func patternIconsView(geometry: GeometryProxy) -> some View {
        ForEach(
            0..<min(20, currentCard.backgroundPatternIcons.count * 5),
            id: \.self
        ) { index in
            Image(
            ...
            .font(
                .system(
                    size: CGFloat.random(
                        in: activeCardIndex == 0 ? 20...40 : 20...30
                    )
                )
            )
            .foregroundColor(
            ...
            .rotationEffect(
                .degrees(isAnimating ? Double.random(in: 0...360) : 0)
            )
            .animation(
                Animation.linear(
                    duration: Double.random(
                        in: activeCardIndex == 0 ? 20...40 : 30...40
                    )
                )
    ...
    private var particlesView: some View {
        ForEach(
            particleSystem.particles.prefix(currentCard.particleCount),
            id: \.id
        ) { particle in
            Circle()
                .fill(
        ...
        Button(action: {
                ...
                Circle()
                    .fill(
                        activeCardIndex == index
                            ? personalCards[index].gradientColors[0]
            ...
            .overlay(
                Circle()
                    ...
                    .scaleEffect(
                        activeCardIndex == index && isPulsingButton ? 1.2 : 1.0
                    )
                    .opacity(
                        activeCardIndex == index && isPulsingButton ? 0 : 1
                    )
                    .animation(
    ...
    private var tapGestureAnimation: some View {
        GeometryReader { geometry in
            ZStack {
                Circle()
                    .fill(Color.white.opacity(0.3))
                    .frame(width: 50, height: 50)
                    .scaleEffect(tapAnimationOffset)
                    .opacity(tapAnimationOffset * 1.5)
        ...
        .onAppear {
            animateTapGestureLoop()
        }
    }

    private func animateTapGestureLoop() {
        tapAnimationOffset = 0
        
        withAnimation(Animation.easeInOut(duration: 1.2).repeatCount(1, autoreverses: true)) {
            tapAnimationOffset = 1.0
        }
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 20)
                    .strokeBorder(
                        LinearGradient(
                            gradient: Gradient(colors: [
                                .white.opacity(0.6), .white.opacity(0),
                            ]),
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        ),
    ...
    private func handleCardDrag(value: DragGesture.Value) {
        ...
        rotationY =
            Double(value.translation.width) / Double(width) * maxRotation * 6
        rotationX =
            -Double(value.translation.height) / Double(height) * maxRotation * 3
    }
    ...
    private func updateAchievementIndex() {
        withAnimation(.easeInOut) {
            activeAchievementIndex =
                (activeAchievementIndex + 1) % currentCard.achievements.count
        }
    }
        ...
        timer = Timer.publish(every: timerInterval, on: .main, in: .common)
            ...
            .sink { _ in
                particleSystem.update(
                    maxSpeed: activeCardIndex == 0 ? 1.5 : 0.8
                )
            }
    }
